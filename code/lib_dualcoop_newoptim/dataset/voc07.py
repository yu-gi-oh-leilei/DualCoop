...
class Voc07Dataset(data.Dataset):
    def __init__(self, 
        ...
        os.makedirs(osp.join(dataset_dir, 'partial_voc_npy', 'label_proportion_{}'.format(label_proportion)), exist_ok=True)
        label_path = osp.join(dataset_dir, 'partial_voc_npy', 'label_proportion_{}'.format(label_proportion), '{}_label_vectors_{}.npy'.format(mode, label_proportion))
        ...
        if label_proportion < 1.0:
            ...
            self.changedLabels = changeLabelProportion(self.labels, label_proportion)
            np.save(label_path, self.changedLabels)
            self.labels = self.changedLabels
